...
import { avocatAPI } from "../services/api.js"; // chemin selon ton projet
...
const ProfilAvo = () => {
  ...
  useEffect(() => {
    const fetchProfil = async () => {
      try {
        const data = await avocatAPI.getMonProfil();
        setProfil(data);
      } catch (error) {
      ...
      <header className="bg-blue-700 text-white shadow-lg">
        <div className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 bg-white text-green-700 hover:bg-gray-200 px-4 py-2 rounded-lg transition font-semibold"
            >
