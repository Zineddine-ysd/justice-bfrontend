...
import { secretaireAPI } from "../services/api.js"; // chemin selon ton projet
...
const ProfilSec = () => {
  ...
  useEffect(() => {
    const fetchProfil = async () => {
      try {
        const data = await secretaireAPI.getMonProfil();
        setProfil(data);
      } catch (error) {
      ...
      <header className="bg-purple-700 text-white shadow-lg">
        <div className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 bg-white text-purple-700 hover:bg-gray-200 px-4 py-2 rounded-lg transition font-semibold"
            >
